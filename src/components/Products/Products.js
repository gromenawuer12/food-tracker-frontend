...
const Products = () => {
  ...
  const getProducts = useCallback(async () => {
    await productCtx.getProducts("products/");
  ...
  const addProduct = async (product) => {
    await productCtx.addProduct("products/", product);
    setDataUpdated(false);
  };

  const deleteProduct = async (name) => {
    await productCtx.deleteProduct("products/", { name: name });
    setDataUpdated(false);
  };

  useEffect(() => {
    if (!nutritionalValueCtx.nutritionalValue) {
      nutritionalValueCtx.getNutritionalValue("nutritional_value/");
    }
  ...
  const addNutritionalValue = (event) => {
    ...
    setNutritionalValue([
      ...nutritionalValue,
      nutritionalValueInputRef.current.value.split(","),
    ]);
  };
          ...
          <form className="px-8 pt-6 pb-8 mb-4 mt-4" onSubmit={addHandler}>
            ...
            <div className="inline-flex mt-4">
              ...
              <div className="relative">
                ...
                >
                  {nutritionalValueCtx.nutritionalValue &&
                    nutritionalValueCtx.nutritionalValue.map((nv) => {
                      return (
                        <option
                          key={nv.name}
                ...
                nutritionalValue.map((nv, index) => {
                  return (
                    ...
                    >
                      ...
                      >
                        <svg
                          class="flex h-4 w-4 text-black justify-end"
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 20 20"
                        >
